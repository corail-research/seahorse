...
from seahorse.tournament.challonge_tournament_2 import ChallongeTournament
...
class ChallongeTournamentTictac(ChallongeTournament) :
    ...
    def build_initial_master(self, p1, p2, game_state, port):
        return MasterTictac(name="Tic-Tac-Toe", initial_game_state=game_state, players_iterator=[p1,p2], log_file=self.log_file, port=port)

    def build_players(self, p1, p2, folder_player) :
        player1_class = __import__(str(folder_player+p1+"_tictac"), fromlist=[None])
        player2_class = __import__(str(folder_player+p2+"_tictac"), fromlist=[None])
